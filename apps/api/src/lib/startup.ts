...
export function validateEnv(): void {
  ...
  for (const check of ENV_CHECKS) {
    ...
    if (!val || val.trim() === '') {
      if (check.required) {
        ...
        warnings.push(
          `  WARN  ${check.key} not set -- ${check.feature ?? check.key} will be unavailable`,
        )
      }
    }
  }
  ...
  if (missing.length) {
    ...
    missing.forEach((k) => console.error(`  x  ${k}`))
...
/** Starts workers in a try/catch so a Redis outage does not kill the HTTP server. */
export async function startWorkersSafely(
  ...
  for (const { name, start } of starters) {
    try {
      start()
      console.info(`[startup] OK ${name} started`)
    } catch (err) {
      console.error(
        `[startup] FAIL ${name} failed to start -- HTTP API will still serve requests:`,
